...
2. -Шаг: Установите `gulp` глобально (т.е. для всей системы): `npm i --global gulp-cli`
...
`npm run dev` -> запускает локальный сервер с проектом.

`npm run build` -> выполняет сборку проекта в директорию **build**
