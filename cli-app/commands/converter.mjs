...
export const describe =
  'Конвертер svg в base64.\nПринимает 2 аргумента: 1. -m или --mode с необходимостью выбора режима работы (single или all); 2. После выбора режима необходимо указать путь используя глобальную опцию -p или --path. Если выбран single - то путь указывается до файла с его расширением. Если all - то указывается путь до дир-рии с файлами svg после конвертации в base64 в этом режиме информация не выводится в консоль, она выводится в текстовый файл!\nНиже приведена полная команда вызванная из корня проекта\n=> node ./cli-app/cli-tools.mjs convert ./src/assets/img/yourSvg.svg';
